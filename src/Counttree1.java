...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for(int p=0;p<quries;p++){
            ...
            List<Integer> list1=nodeSet(sampleArr,node1,node2);
                //shortestPath(g,node1,nodes,node2);
            /*for(int listVal:list1){
                System.out.println(listVal);
            }
            System.out.println("-----------------------");
            System.out.println("First The nodes btw ::"+node3+" , "+node4);*/
            List<Integer> list2=nodeSet(sampleArr,node3,node4);
                //shortestPath(g,node3,nodes,node4);
    ...
    static List<Integer> nodeSet(int[] arr,int parent,int child){
        int temp=child;
      
        List<Integer> sampleList=new ArrayList<Integer>();
        while(temp!=parent && temp!=0){
             sampleList.add(temp);
            int sample=arr[temp];
        ...
        if(temp==0){
            //System.out.println("What you have done is wrong");
            List<Integer> sampleList2=new ArrayList<Integer>();
            int temp2=parent;
            
            while(temp2!=child && temp2!=0){
                sampleList2.add(temp2);
                int tempSample=arr[temp2];
                temp2=tempSample;
                if(sampleList.contains(temp2)){
                    break;
                }
            ...
            for(int i=0;i<sampleList.size();i++){
                if(sampleList.get(i)!=temp2){
                sampleList2.add(sampleList.get(i));
                }else{
                    break;
                }
            }
     ...
     static List<Integer> shortestPath(Graph graph,int rootNode,int vertices,int dest){
        ...
        /* for(Integer pathVal: pathList) {
         ...
         System.out.println("");*/
              System.out.println("Out of Shortest path algoritm");
              return pathList; 
    }
